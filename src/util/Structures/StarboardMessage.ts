...
export class StarboardMessage implements SM {
  ...
  constructor(
    public guild: Guild,
    public sb: Starboard,
    public message: StarboardMessageRaw
  ) {
    this.assign().catch(console.error);
  }
  ...
  private async assign(): Promise<void> {
    this.msg =
      ...
      ) as TextChannel).messages.cache.get(this.message.msg) ||
      (await (this.guild.channels.cache.get(
        this.message.channel
      ) as TextChannel).messages.fetch(this.message.msg));
    this.content = this.msg.content;
    this.sbMsg =
      this.sb.channel.messages.cache.get(this.message.sbMsg) ||
      (await this.sb.channel.messages.fetch(this.message.sbMsg)) ||
      null;
    this.attachments = this.msg?.attachments?.array();
    this.author =
      this.guild.members.cache.get(this.message.author) ||
  ...
  async updateCount(count: number): Promise<void> {
    ...
    await this.guild.client.db.updateStarCount(
      this.guild.id,
      this.msg.id,
      count
    );
