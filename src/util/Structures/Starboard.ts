...
import {
  StarboardMessage
} from '@util/Structures/StarboardMessage'
...
export class Starboard implements SB {
  ...
  private assign (): void {
    ...
    this.channel = this.guild.channels.cache.find(({ id, type }) => id === this.ops.channel && type === 'text') as TextChannel
    this.emoji = this.guild.emojis.cache.get(this.ops.emoji) ||
      this.ops.emoji
    ...
    for (const chan of this.ops.excludeChannels) {
      const ch = this.guild.channels.cache.find(({ id, type }) => id === chan && type === 'text') as TextChannel
  ...
  save (): void {
    this.guild.client.db.addStarboard(this.guild.id, this)
      .then(console.log)
  }
  ...
  toRaw (): StarboardRaw {
    return {
      ...
      emoji: this.emoji instanceof GuildEmoji ? this.emoji.id || this.emoji.name : this.emoji,
