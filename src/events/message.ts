...
import { Message, PermissionResolvable, GuildMember, Permissions } from 'discord.js'
...
export = async (client: Client, message: Message) => {
  ...
  if (user.toArray().length || bot.toArray().length) {
    const m: string[] = [':x: The command could not be preformed because one or more permissions are missing.']

    if (user.toArray().length) m.push('You are missing:', ...user.toArray(false).map((p) => `> \`${p as string}\``), `**Required**: \`${user.bitfield}\``)
    if (bot.toArray().length) m.push('I am missing:', ...bot.toArray(false).map((p) => `> \`${p as string}\``), `**Required**: \`${bot.bitfield}\``)
 ...
 */
function checkPermissions ({ config: { permissions } }: Command, { guild: { me: { permissions: botPerms } }, permissions: userPerms }: GuildMember): { user: Permissions, bot: Permissions } {
  ...
  if (!userPerms.has(user)) missing.user = new Permissions(userPerms.missing(user))
